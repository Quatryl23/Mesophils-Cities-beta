package net.mesomods.mesophilscities.world.teleporter;
...
import net.mesomods.mesophilscities.procedures.CheckForPortalHouse240Procedure;
...
public class CityTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("city_portal", Vec3i::compareTo, 300);
	...
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(MesophilsCitiesModBlocks.CITY_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register("city_portal", poiType);
	...
	public CityTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	public Optional<BlockUtil.FoundRectangle> findPortalAround(BlockPos p_192986_, boolean p_192987_, WorldBorder p_192988_) {
        ...
        if (this.level.dimension().equals(CITY)) {
        	ListTag portal = CheckForPortalHouse240Procedure.execute(this.level, p_192986_.getX(),p_192986_.getZ());
        	p_192986_ = new BlockPos(((IntTag) portal.get(0)).getAsInt(), p_192986_.getY(), ((IntTag) portal.get(1)).getAsInt());
        } else {
        	p_192986_ = new BlockPos((int)Math.floor((p_192986_.getX()-128)/240.0)*240+240,p_192986_.getY(),(int)Math.floor(((p_192986_.getZ()-128)/240.0))*240+240);
        	i = 344;
        	int SearchMinX = p_192986_.getX()-i;
        	int SearchMinZ = p_192986_.getZ()-i;
        	int SearchMaxX = p_192986_.getX()+i;
        	int SearchMaxZ = p_192986_.getZ()+i;
        	// System.out.println("[Mesophils Cities Portal Debug] Searching Portal in area from pos x: " + SearchMinX + " z: " + SearchMinZ + " to pos x: " + SearchMaxX + " z: " + SearchMaxZ + " ");
        }

        	BlockPos finalp_192986_ = p_192986_;
		...
		Optional<PoiRecord> optional = poimanager.getInSquare((p_230634_) -> {
			...
			return p_192984_.getPos().distSqr(finalp_192986_);
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		for (BlockPos.MutableBlockPos blockpos$mutableblockpos1 : BlockPos.spiralAround(p_77667_, 16, Direction.EAST, Direction.SOUTH)) {
			int j = Math.min(i, this.level.getHeight(Heightmap.Types.MOTION_BLOCKING, blockpos$mutableblockpos1.getX(), blockpos$mutableblockpos1.getZ())+8);
			int k = 1;
		...
		if (d0 == -1.0D) {
			...
			ListTag portal = CheckForPortalHouse240Procedure.execute(this.level, p_77667_.getX(),p_77667_.getZ());
			blockpos = new BlockPos((((IntTag) portal.get(0)).getAsInt()),(int) 64, (((IntTag) portal.get(1)).getAsInt()));
			if (((IntTag) portal.get(2)).getAsInt() < 2) {
				p_77668_ = Direction.Axis.X;
			} else {
				p_77668_ = Direction.Axis.Z;
			}
			direction = Direction.get(Direction.AxisDirection.POSITIVE, p_77668_);
		...
		BlockState blockstate = MesophilsCitiesModBlocks.CITY_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = CityPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(), entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return CityPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity, entity.getDeltaMovement(), entity.getYRot(), entity.getXRot());
