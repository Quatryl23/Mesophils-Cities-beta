...
public class PathPlacementNearStreetsSouthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		ListTag place_block;
		sizexz_pos_xz = DataCalculationPositionHouseblockProcedure.execute(x, z + 48);
		...
		path_e = path;
		if (path_e > 0) {
			testnumbr = 16 + 4 * HouseDistanceZProcedure.execute(x, z);
			for (int index0 = 0; index0 < (int) (4 - HouseDistanceZProcedure.execute(x, z)); index0++) {
				world.setBlock(BlockPos.containing(x - 20 + 4 * path_e, y, z + testnumbr), Blocks.DEEPSLATE_TILES.defaultBlockState(), 3);
