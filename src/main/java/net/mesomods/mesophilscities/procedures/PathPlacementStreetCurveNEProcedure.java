...
public class PathPlacementStreetCurveNEProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (path_s != 0) {
			testnumbr = z - 20 + 4 * (border2_w != 0 ? border2_w : path_w + 1);
			for (int index1 = 0; index1 < (int) (13 - (border2_w != 0 ? border2_w : path_w + 1)); index1++) {
		...
		if (path_w != 0) {
			testnumbr = x - 16;
			for (int index2 = 0; index2 < (int) (border1_s != 0 ? border1_s : path_s); index2++) {
				world.setBlock(BlockPos.containing(testnumbr, y, z - 20 + 4 * path_w), Blocks.DEEPSLATE_TILES.defaultBlockState(), 3);
				testnumbr = testnumbr + 4;
			}
		}
		testnumbrx = border1_s != 0 ? border1_s : path_s;
		testnumbr = (path_s != 0) ? (border1_s != 0) ? (path_s + 1) : border2_s : (border3_s != 0) ? border3_s : border2_s;
		while (testnumbrx < testnumbr) {
			testnumbrz = (path_w != 0) ? (border2_w != 0) ? path_w : border1_w : (border3_w != 0) ? border3_w : border1_w;
			while (testnumbrz < (border2_w != 0 ? border2_w : path_w + 1)) {
				if (testnumbrx < 5 - HouseDistanceXProcedure.execute(x, z) || testnumbrz > 8 + HouseDistanceZProcedure.execute(x, z)) {
