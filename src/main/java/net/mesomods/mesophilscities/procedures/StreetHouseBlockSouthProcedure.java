...
public class StreetHouseBlockSouthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, double id) {
		...
		if (border1_s > 0) {
			if (id == 7 && border1_s < 5 || id == 8 && border1_s >= 5) {
				assert Boolean.TRUE; //#dbg:StreetHouseBlockSouth:C
				...
				BlockPlacementHousesProcedure.execute(world, x, y, z, false, true, TrainStreetCalculationProcedure.execute(x, z + 48) != 0 || border1_e2 == 0, path_s > 0, border2_s, path_s > 0 ? path_s + 1 : border3_s,
						(TrainStreetCalculationProcedure.execute(x, z + 48) == 9)
								...
								: (TrainStreetCalculationProcedure.execute(x, z + 48) == 10)
										...
										: (TrainStreetCalculationProcedure.execute(x, z + 48) != 0)
												? (17 - HouseDistanceZProcedure.execute(x, z + 48))
												: PathDirectionProcedure.execute(x, z + 48) ? (path_e2 > 0) ? (Random3Procedure.execute(x, z + 48) + 12) : (path_e2 + 12) : (border1_e2 > 0) ? (border1_e2 + 12) : (path_e2 + 12),
						9 + HouseDistanceZProcedure.execute(x, z));
			}
		}
		if (id == 9) {
			assert Boolean.TRUE; //#dbg:StreetHouseBlockSouth:I
			BlockPlacementHousesProcedure.execute(world, x, y, z, border1_s2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 8 > 3, true, border1_e2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 2 == 1,
					border2_s == 0, border1_s2 > 0 ? border1_s2 + 12 : path_s2 + 12, border2_s > 0 ? border2_s : path_s + 1, border1_e2 > 0 ? border1_e2 + 12 : path_e2 + 12, 9 + HouseDistanceZProcedure.execute(x, z));
		}
	}
}
