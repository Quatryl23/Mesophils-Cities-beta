...
public class CheckForPortalHousePlacementFalseProcedure {
	public static ListTag execute(double x, double z) {
		...
		if (border1_n > 0 && border1_s > 0) {
			if (border1_w > 0) {
				portal_house = CheckForPortalHouseProcedure.execute(x, z, true, false, path_w > 0 ? true : false, false, path_n, Random1Procedure.execute(x, z), (path_w > 0) ? path_w : (border3_w > 0) ? border3_w : border2_w, border1_w);
				if (!portal_house.isEmpty()) {
					return portal_house;
				}
			}
			if (border2_w > 0 && (border3_w > 0 || path_w > 0)) {
				portal_house = CheckForPortalHouseProcedure.execute(x, z, true, path_w > 0 ? true : false, false, false, path_n, Random3Procedure.execute(x, z), border2_w, path_w > 0 ? path_w + 1 : border3_w);
				if (!portal_house.isEmpty()) {
					return portal_house;
				}
			}
			portal_house = CheckForPortalHouseProcedure.execute(x, z, true, border2_w == 0, TrainStreetCalculationProcedure.execute(x, z + 48) != 0 || border1_w2 == 0, false, path_s, border1_s,
		...
		if (border2_n > 0 && border2_s > 0) {
			if (border1_e > 0) {
				portal_house = CheckForPortalHouseProcedure.execute(x, z, false, false, path_e > 0 ? true : false, true, Random2Procedure.execute(x, z), path_n + 1, (path_e > 0) ? path_e : (border3_e > 0) ? border3_e : border2_e, border1_e);
				if (!portal_house.isEmpty()) {
					return portal_house;
				}
			}
			if (border2_e > 0 && (border3_e > 0 || path_e > 0)) {
				portal_house = CheckForPortalHouseProcedure.execute(x, z, false, path_e > 0 ? true : false, false, true, Random4Procedure.execute(x, z), path_s + 1, border2_e, path_e > 0 ? path_e + 1 : border3_e);
				if (!portal_house.isEmpty()) {
					return portal_house;
				}
			}
			portal_house = CheckForPortalHouseProcedure.execute(x, z, false, border2_e == 0, TrainStreetCalculationProcedure.execute(x, z + 48) != 0 || border1_e2 == 0, true, border2_s, path_s + 1,
					(TrainStreetCalculationProcedure.execute(x, z + 48) == 9)
		...
		if (border1_e > 0) {
			portal_house = CheckForPortalHouseProcedure.execute(x, z, TrainStreetCalculationProcedure.execute(x + 48, z) != 0 || border1_n2 == 0, false, path_e > 0, border2_n == 0,
		...
		if (border2_e > 0 && (border3_e > 0 || path_e > 0)) {
			portal_house = CheckForPortalHouseProcedure.execute(x, z, TrainStreetCalculationProcedure.execute(x + 48, z) != 0 || border1_s2 == 0, path_e > 0, false, border2_s == 0,
					(TrainStreetCalculationProcedure.execute(x + 48, z) == 6)
