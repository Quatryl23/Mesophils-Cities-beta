...
public class DataPlacementHousesCurveNWProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, double id) {
		...
		if (border1_s > 0) {
			if (id == 7 && border1_s < 5 || id == 8 && border1_s >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveNW:G
				BlockPlacementHousesProcedure.execute(world, x, y, z, path_s > 0, true, TrainStreetCalculationProcedure.execute(x, z + 48) != 0 || border1_w2 == 0, false, (path_s > 0) ? path_s : (border3_s > 0) ? border3_s : border2_s, border1_s,
						...
						(border2_s != 0 && (border3_s != 0 || path_s != 0)) ? (9 + HouseDistanceZProcedure.execute(x, z)) : (border2_e > 0) ? border2_e : (path_e + 1));
			}
		}
		if (border2_s > 0 && (border3_s > 0 || path_s > 0)) {
			if (id == 7 && (path_s > 0 ? path_s + 1 : border3_s) < 5 || id == 8 && (path_s > 0 ? path_s + 1 : border3_s) >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveNW:H
				BlockPlacementHousesProcedure.execute(world, x, y, z, false, true, TrainStreetCalculationProcedure.execute(x, z + 48) != 0 || border1_e2 == 0, path_s > 0, border2_s, path_s > 0 ? path_s + 1 : border3_s,
						(TrainStreetCalculationProcedure.execute(x, z + 48) == 9)
		...
		if (border1_e > 0) {
			if (id == 3 && border1_e < 5 || id == 6 && border1_e >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveNW:C
						...
						(TrainStreetCalculationProcedure.execute(x + 48, z) == 6)
								...
								: (TrainStreetCalculationProcedure.execute(x + 48, z) == 10)
										...
										: (TrainStreetCalculationProcedure.execute(x + 48, z) != 0)
												...
												: (!PathDirectionProcedure.execute(x + 48, z)) ? (border1_n2 > 0) ? (Random1Procedure.execute(x + 48, z) + 12) : (path_n2 + 12) : (border1_n2 > 0) ? (border1_n2 + 12) : path_n2,
						(border2_e != 0 && (border3_e != 0 || path_e != 0)) ? (9 + HouseDistanceXProcedure.execute(x, z)) : (border2_s != 0) ? border2_s : (path_s + 1), (path_e > 0) ? path_e : (border3_e > 0) ? border3_e : border2_e, border1_e);
			}
		}
		if (border2_e > 0 && (border3_e > 0 || path_e > 0)) {
			if (id == 3 && (path_e > 0 ? path_e + 1 : border3_e) < 5 || id == 6 && (path_e > 0 ? path_e + 1 : border3_e) >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveNW:F
				BlockPlacementHousesProcedure.execute(world, x, y, z, TrainStreetCalculationProcedure.execute(x + 48, z) != 0 || border1_s2 == 0, path_e > 0, false, true,
						...
						border2_s > 0 ? border2_s : path_s + 1, border2_e, path_e > 0 ? path_e + 1 : border3_e);
			}
		}
		if (id == 9) {
			assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveNW:I
			BlockPlacementHousesProcedure.execute(world, x, y, z, border1_s2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 8 > 3, border2_e == 0, border1_e2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 2 == 1,
					border2_s == 0, border1_s2 > 0 ? border1_s2 + 12 : path_s2 + 12, border2_s > 0 ? border2_s : path_s + 1, border1_e2 > 0 ? border1_e2 + 12 : path_e2 + 12, border2_e > 0 ? border2_e : path_e + 1);
		}
	}
}
