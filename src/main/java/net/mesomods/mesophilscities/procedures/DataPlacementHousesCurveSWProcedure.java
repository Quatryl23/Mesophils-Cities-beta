...
public class DataPlacementHousesCurveSWProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, double id) {
		...
		if (border1_e > 0) {
			if (id == 3 && border1_e < 5 || id == 6 && border1_e >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveSW:C
				BlockPlacementHousesProcedure.execute(world, x, y, z, TrainStreetCalculationProcedure.execute(x + 48, z) != 0 || border1_n2 == 0, false, path_e > 0, true,
						...
						border2_n > 0 ? border2_n : path_n + 1, (path_e > 0) ? path_e : (border3_e > 0) ? border3_e : border2_e, border1_e);
			}
		}
		if (border2_e > 0 && (border3_e > 0 || path_e > 0)) {
			if (id == 3 && (path_e > 0 ? path_e + 1 : border3_e) < 5 || id == 6 && (path_e > 0 ? path_e + 1 : border3_e) >= 5) {
				assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveSW:F
				BlockPlacementHousesProcedure.execute(world, x, y, z, TrainStreetCalculationProcedure.execute(x + 48, z) != 0 || border1_s2 == 0, path_e > 0, false, true,
						...
						(border1_e != 0) ? (9 + HouseDistanceXProcedure.execute(x, z)) : (border2_n != 0) ? border2_n : (path_n + 1), border2_e, path_e > 0 ? path_e + 1 : border3_e);
			}
		}
		if (id == 9) {
			assert Boolean.TRUE; //#dbg:DataPlacementHousesCurveSW:I
			BlockPlacementHousesProcedure.execute(world, x, y, z, border1_s2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 8 > 3, border2_e == 0, border1_e2 == 0 || TrainStreetCalculationProcedure.execute(x + 48, z + 48) % 2 == 1,
					true, border1_s2 > 0 ? border1_s2 + 12 : path_s2 + 12, border2_s > 0 ? border2_s : path_s + 1, border1_e2 > 0 ? border1_e2 + 12 : path_e2 + 12, border2_e > 0 ? border2_e : path_e + 1);
		}
	}
}
