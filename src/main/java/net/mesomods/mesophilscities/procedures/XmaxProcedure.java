...
public class XmaxProcedure {
	public static double execute(double x, double z) {
		...
		return Math.max(seednorth < 10 ? seednorth : Math.max(Math.floor(seednorth / 10), seednorth % 10), seedsouth < 10 ? seedsouth : Math.max(Math.floor(seedsouth / 10), seedsouth % 10));
	}
}
