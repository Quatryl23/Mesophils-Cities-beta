...
public class PathPlacementStreetCurveNWProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (path_s > 0) {
			testnumbr = z - 20 + 4 * (border2_e != 0 ? border2_e : path_e + 1);
			for (int index1 = 0; index1 < (int) (13 - (border2_e != 0 ? border2_e : path_e + 1)); index1++) {
				world.setBlock(BlockPos.containing(x - 20 + 4 * path_s, y, testnumbr), Blocks.DEEPSLATE_TILES.defaultBlockState(), 3);
				testnumbr = testnumbr + 4;
			}
		}
		if (path_e > 0) {
			testnumbr = x - 20 + 4 * (border2_s != 0 ? border2_s : path_s + 1);
			for (int index2 = 0; index2 < (int) (13 - (border2_s != 0 ? border2_s : path_s + 1)); index2++) {
				world.setBlock(BlockPos.containing(testnumbr, y, z - 20 + 4 * path_e), Blocks.DEEPSLATE_TILES.defaultBlockState(), 3);
				testnumbr = testnumbr + 4;
			}
		}
		testnumbrx = (path_s != 0) ? (border2_s != 0) ? path_s : border1_s : (border3_s != 0) ? border3_s : border1_s;
		while (testnumbrx < (border2_s != 0 ? border2_s : path_s + 1)) {
			testnumbrz = (path_e != 0) ? (border2_e != 0) ? path_e : border1_e : (border3_e != 0) ? border3_e : border1_e;
			while (testnumbrz < (border2_e != 0 ? border2_e : path_e + 1)) {
				if (testnumbrx > 8 + HouseDistanceXProcedure.execute(x, z) || testnumbrz > 8 + HouseDistanceZProcedure.execute(x, z)) {
