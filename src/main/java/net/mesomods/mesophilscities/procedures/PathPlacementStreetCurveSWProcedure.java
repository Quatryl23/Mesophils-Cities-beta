...
public class PathPlacementStreetCurveSWProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (path_n != 0) {
			testnumbr = z - 16;
			for (int index1 = 0; index1 < (int) (border1_e != 0 ? border1_e : path_e); index1++) {
		...
		if (path_e != 0) {
			testnumbr = x - 20 + 4 * (border2_n != 0 ? border2_n : path_n + 1);
			for (int index2 = 0; index2 < (int) (13 - (border2_n != 0 ? border2_n : path_n + 1)); index2++) {
				world.setBlock(BlockPos.containing(testnumbr, y, z - 20 + 4 * path_e), Blocks.DEEPSLATE_TILES.defaultBlockState(), 3);
				testnumbr = testnumbr + 4;
			}
		}
		testnumbrx = (path_n != 0) ? (border2_n != 0) ? path_n : border1_n : (border3_n != 0) ? border3_n : border1_n;
		testnumbr = (path_e != 0) ? (border1_e != 0) ? (path_e + 1) : border2_e : (border3_e != 0) ? border3_e : border2_e;
		while (testnumbrx < (border2_n != 0 ? border2_n : path_n + 1)) {
			testnumbrz = border1_e != 0 ? border1_e : path_e;
			while (testnumbrz < testnumbr) {
				if (testnumbrx > 8 + HouseDistanceXProcedure.execute(x, z) || testnumbrz < 5 - HouseDistanceZProcedure.execute(x, z)) {
