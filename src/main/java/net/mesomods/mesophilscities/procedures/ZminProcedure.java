...
public class ZminProcedure {
	public static double execute(double x, double z) {
		...
		return Math.min(seedwest < 10 ? seedwest : Math.min(Math.floor(seedwest / 10), seedwest % 10), seedeast < 10 ? seedeast : Math.min(Math.floor(seedeast / 10), seedeast % 10));
	}
}
